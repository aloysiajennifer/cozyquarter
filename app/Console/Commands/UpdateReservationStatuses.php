...
class UpdateReservationStatuses extends Command
{
   ...
   public function handle()
   {
       ...
       try {
           ...
           $timeThresholdForNotAttended = $now->format('Y-m-d H:i:s'); // Current time
          
           $notAttendedReservations = Reservation::where('status_reservation', Reservation::STATUS_RESERVED)
               ->whereNull('check_in_time')
               // Condition: Current time is 20 minutes or more past the reservation_start_time
           ...
           foreach ($notAttendedReservations as $reservation) {
               ...
               $reservation->save();
               $justNotAttendedIds[] = $reservation->id; // Add ID to skip list for this run's 'Closed' logic
               ...
               if ($user) {
                   $user->increment('penalty_counter'); // Increment penalty_counter
                   Log::info("User {$user->email}'s penalty_counter incremented to {$user->penalty_counter}.");


                   // If user is now blacklisted (based on penalty_counter >= 3)
           ...
           // - Already Cancelled (3)
           // - Already Closed (4)
           // - OR were just marked as Not Attended in THIS current run
           $closedReservationsQuery = Reservation::query()
               ->where('status_reservation', '!=', Reservation::STATUS_CANCELLED) // Exclude Cancelled
               ->where('status_reservation', '!=', Reservation::STATUS_CLOSED)   // Exclude already Closed
               ->whereRaw("CONCAT(reservation_date, ' ', reservation_end_time) <= ?", [$now->format('Y-m-d H:i:s')]); // Time has passed


           // Crucially, exclude reservations that just became Not Attended in this same run
