...
<div class="w-full p-4 min-h-screen">
    ...
    <form action="{{ route('book.index') }}" method="GET" class="max-w-md mx-auto mb-4">   
        ...
        <div class="relative">
            ...
            <button type="submit"
                class="bg-[var(--accent-blue)] text-[var(--primary)] hover:bg-[var(--accent-green)] focus:ring-4 focus:ring-[var(--accent-green)] focus:outline-none font-medium rounded-lg text-sm px-4 py-2 absolute right-2.5 bottom-2.5">
                Search
            </button>
        </div>
    </form>
    ...
    @if($alertType === 'error')
        <div id="alert-error" class="flex items-center p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50" role="alert">
            <svg class="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm.75 14.25a.75.75 0 0 1-1.5 0V11a.75.75 0 0 1 1.5 0Zm0-6a.75.75 0 0 1-1.5 0V7a.75.75 0 0 1 1.5 0Z"/>
            </svg>
    ...
    @elseif($alertType === 'info')
        <div id="alert-info" class="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50" role="alert">
            <svg class="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm.75 14.25a.75.75 0 0 1-1.5 0V11a.75.75 0 0 1 1.5 0Zm0-6a.75.75 0 0 1-1.5 0V7a.75.75 0 0 1 1.5 0Z"/>
            </svg>
...
@endif
        ...
        <table class="w-full text-sm text-left text-gray-500 bg-white border border-gray-200 rounded-lg shadow-md">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b hover:bg-gray-100">
                    ...
                    <td class="px-6 py-4 font-medium text-[var(--primary)] whitespace-nowrap">{{ $book->title_book }}</td>
                    ...
                    <td class="px-6 py-4 max-w-[200px] overflow-hidden text-ellipsis whitespace-nowrap text-[var(--primary)]">
                        {{ $book->synopsis_book }}
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <a href="{{ url('book/detail/' . encrypt($book->id)) }}">
                            <button type="button" class="bg-[var(--accent-blue)] text-[var(--primary)] hover:bg-[var(--accent-green)] focus:ring-4 focus:ring-[var(--accent-green)] font-medium rounded-lg text-sm px-5 py-2.5 me-2">
                                Edit
                            </button>
                        </a>
                    </td>
                    <td class="px-6 py-4">
                        <form action="{{ route('book.delete', ['id'=> encrypt($book->id)]) }}" method="POST" style="display:inline;">
                            @csrf
                            <button type="button" class="delete-button text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5"
    ...
    <div class="mb-4 mt-4 text-right">
        <a href="{{ url('/book/form') }}">
            <button class="bg-[var(--accent-blue)] text-[var(--primary)] hover:bg-[var(--accent-green)] focus:ring-4 focus:ring-[var(--accent-green)] font-medium rounded-lg text-sm px-5 py-2.5">
                Add
            </button>
        </a>
    </div>
